...
export const FairnessNote: React.FC<Props> = ({ fairness, lang = 'en' }) => {
  ...
  const indicatorColor = isRobust ? 'text-accent' : 'text-warning';
  const indicatorBorder = isRobust ? 'border-accent-border bg-bg-card' : 'border-warning-border bg-bg-card';
  ...
  return (
    ...
    >
      ...
      <p className="text-[12px] text-text mt-1.5 border-l-2 border-border pl-2.5">
        {lang === 'zh'
