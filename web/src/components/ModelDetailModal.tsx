    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose} role="dialog" aria-modal="true">
      ...
      >
        <div className="flex justify-between items-start">
          ...
          <button onClick={onClose} className="text-[#717771] hover:text-white p-1" aria-label="Close"><X size={18} /></button>
        </div>

        <div className="mt-5 grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">CO₂ (mid + range)</div>
            <div className="font-mono text-xl font-semibold mt-0.5">{formatCO2Range(model.co2_kg)}</div>
          </div>
          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">Water</div>
            <div className="font-mono text-xl font-semibold mt-0.5">{formatWaterRange(model.water_liters)}</div>
          </div>

          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">Efficiency</div>
            <div className="font-mono mt-0.5">{formatCO2Per1kG(eff)}</div>
          </div>
          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">Origin / Type</div>
            <div className="mt-0.5">{model.origin} · {model.open_or_closed}</div>
          </div>

          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">Energy per output token (Wh)</div>
          ...
          <div>
            <div className="text-[#717771] text-xs uppercase tracking-widest">PUE × Grid (gCO₂/kWh)</div>
        ...
        {model.flags.length > 0 && (
          <div className="mt-4">
            ...
            <p className="text-[11px] text-[#717771] mt-2">FALLBACK flags indicate conservative parameter-class or annual grid assumptions. See methodology for full sensitivity.</p>
          </div>
        )}
